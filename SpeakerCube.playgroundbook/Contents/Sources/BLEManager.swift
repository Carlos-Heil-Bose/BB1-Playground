...
public let BLEPoweredOnNotification                 = "kBLEPoweredOn"
public let BLEPoweredOffNotification                = "kBLEPoweredOff"
public let BLEBMAPServiceAvailableNotification      = "kBLEBMAPServiceAvailable"
public let BLEBOSEbuildDeviceFoundNotification      = "kBLEBOSEbuildDeviceFound"
public let BLEBOSEbuildDeviceConnectedNotification  = "kBLEBOSEbuildDeviceConnected"
...
open class BLEManager : NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    let BOSE_UUID:[CBUUID]         = [CBUUID(string:BOSE_SERVICE_UUID)]

    ///@brief BMAP characteristic UUID
    let BMAP_CHARACTERISTIC:CBUUID =  CBUUID(string:BMAP_CHARACTERISTIC_UUID)
    ...
    /// @brief List of BOSEbuild devices found during the scan period and their
    /// associated RSSI
    var BOSEbuildDevicesFound = [String() : Int()]

    /// @brief List of BOSEbuild devices found with their name and associated CBPeripheral
    /// instance
    var BOSEbuildPeripheral   = [String : CBPeripheral]()

    fileprivate let STORED_BB_DEVICE_ID = "STORED_BB_DEVICE_ID"
    ...
    open func ConnectToDevice(_ DeviceName:String) {
        ...
        // **** This doesn't work with the iPad Playground app ***
        // **** No clue as to why ***
    ...
    open func centralManagerDidUpdateState(_ central: CBCentralManager) {
        ...
        if (central.state == .poweredOff) {
            ...
            userDefaults.removeObject(forKey:STORED_BB_DEVICE_ID)
            userDefaults.synchronize();
            NSLog("CoreBluetooth BLE hardware is powered off")
        }
        else if (central.state == .poweredOn) {
            let userDefaults = UserDefaults.standard
            let peripheralUUID = userDefaults.string(forKey: STORED_BB_DEVICE_ID)
            if (peripheralUUID != nil) {
                let uuid = NSUUID(uuidString: peripheralUUID! as String)
                let data =  central.retrievePeripherals(withIdentifiers: [uuid as! UUID]) as [CBPeripheral]
                if data.count > 0 {
                    self.peripheral = data[0]
                    print("Retrieved peripheral = \(self.peripheral)")
    ...
    open func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        ...
        // (should be only the Bose service (0xFEBE) since we only tried to
        ...
        for service in peripheral.services! {
            ...
            for characteristic in service.characteristics! {
                ...
                if (characteristic.uuid == BMAP_CHARACTERISTIC) {
                    ...
                    let userDefaults = UserDefaults.standard
                    userDefaults.set(peripheral.identifier.uuidString as String, forKey: STORED_BB_DEVICE_ID)
                    userDefaults.synchronize()
